...
struct Args {
    ...
    const char* json_messages = nullptr;
...
static Args parse_args(int argc, char* argv[], int start) {
    ...
    for (int i = start; i < argc; i++) {
        if (strcmp(argv[i], "--model") == 0 && i + 1 < argc) {
            ...
            args.prompt = argv[++i];
        } else if (strcmp(argv[i], "--json-messages") == 0 && i + 1 < argc) {
            args.json_messages = argv[++i];
...
static int cmd_generate(LLMModel& model, Tokenizer& tokenizer, const Args& args) {
    ...
    if (args.json_messages) {
        try {
            auto j = nlohmann::json::parse(args.json_messages);
            for (auto& item : j) {
                messages.push_back({item["role"].get<std::string>(), item["content"].get<std::string>()});
            }
        } catch (const std::exception& e) {
            fprintf(stderr, "Error parsing JSON messages: %s\n", e.what());
            return 1;
        }
    } else {
        messages.push_back({"user", args.prompt});
    }
    ...
    // Output JSON stats to stdout for easy parsing by server wrapper
    nlohmann::json stats;
    stats["prompt_tokens"] = last.prompt_tokens;
    stats["generation_tokens"] = last.generation_tokens;
