...
void Qwen35Model::reset(int n_keep) {
    for (int L = 0; L < num_layers_; L++) {
        if (layer_types_[L] == LayerType::FullAttention) {
            ...
            if (n_keep < 0) {
                kv.len = 0;
                kv.start = 0;
                memset(kv.k_cache, 0, (size_t)KV_CACHE_CAPACITY * num_kv_heads_ * head_dim_ * sizeof(float));
                memset(kv.v_cache, 0, (size_t)KV_CACHE_CAPACITY * num_kv_heads_ * head_dim_ * sizeof(float));
            } else {
                int keep = n_keep;
                if (keep > kv.capacity) {
                    keep = kv.capacity;
        ...
        if (layer_types_[L] == LayerType::LinearAttention) {
            ...
            // DeltaNet (RNN-style) doesn't support easy "backtracking" to n_keep.
            // If n_keep is non-zero, warn and keep current recurrent state unchanged.
            if (n_keep != 0) {
                fprintf(stderr, "Warning: LinearAttention layer %d does not support partial KV retention (n_keep=%d); recurrent state is not rewound.\n", L, n_keep);
            }
            if (n_keep == 0) {
                memset(st.ssm_state, 0, (size_t)lin_num_heads_ * lin_key_dim_ * lin_val_dim_ * sizeof(float));
                memset(st.conv_state, 0, (size_t)lin_qkv_dim_ * (conv_kernel_ - 1) * sizeof(float));
                st.conv_pos = 0;
            }
        }
    }
